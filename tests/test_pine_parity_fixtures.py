...
@pytest.mark.parametrize("missing_open_bar", [False, True])
def test_xnys_preopen_window_clears_at_its_row_local_open(missing_open_bar):
    utc_preopen = WindowSpec(
        "utc_preopen", "UTC", time(14), time(15),
        "xnys_session", "standard")
    bars = _local_spans("America/New_York", [
        ("2026-02-02", "08:30", "10:30"),
        ("2026-02-03", "08:30", "10:30"),
    ])
    utc = bars.index.tz_convert("UTC")
    if missing_open_bar:
        bars = bars[~(
            (utc.date == pd.Timestamp("2026-02-03").date())
            & (utc.hour == 14) & (utc.minute == 0)
        )]
        utc = bars.index.tz_convert("UTC")
    first_window = (
        (utc.date == pd.Timestamp("2026-02-02").date())
        & (utc.hour == 14)
    )
    second_window = (
        (utc.date == pd.Timestamp("2026-02-03").date())
    ...
    bars.loc[first_window, "high"] = 110.0
    bars.loc[second_window, "high"] = 120.0
    bars.loc[bars.index[second_window][0], "high"] = 125.0
    vocabulary = core_vocabulary().with_windows(utc_preopen)
    node = {
        "ind": "highest", "of": {"src": "high"},
        "window": "utc_preopen",
    }
    engine = FrameEval(
        {"15m": bars}, vocabulary=vocabulary).series(node, "15m")
    pine = _pine(node, bars, vocabulary)
    preopen = second_window & ((utc.minute == 0) | (utc.minute == 15))
    close = (
        (utc.date == pd.Timestamp("2026-02-03").date())
        & (utc.hour == 15) & (utc.minute == 0)
    )
    assert engine[preopen].isna().all()
    assert pine[preopen].isna().all()
    assert engine[close].eq(125.0).all()
    assert pine[close].eq(125.0).all()
    np.testing.assert_allclose(
        pine.to_numpy(), engine.to_numpy(dtype="float64"),
        rtol=0.0, atol=0.0, equal_nan=True)
